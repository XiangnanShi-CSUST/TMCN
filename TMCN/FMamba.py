...
class CrossMambaFusionBlock(nn.Module):
    ...
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        # print("x_rgb_cross,_forward每个维度的大小为:", x_rgb_cross.shape)
        # print("x_e_cross,_forward每个维度的大小为:", x_e_cross.shape)
        x_rgb = x_rgb + self.drop_path1(x_rgb_cross)
        # print("x_rgb,_forward每个维度的大小为:", x_rgb.shape)
        x_e = x_e + self.drop_path2(x_e_cross)
        # print("x_e,_forward每个维度的大小为:", x_e.shape)
        return x_rgb, x_e
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        if self.use_checkpoint:
            ...
            # print("x_rgb,CrossMambaFusionBlock每个维度的大小为:", x_rgb.shape)
            # print("x_e,CrossMambaFusionBlock每个维度的大小为:", x_e.shape)
...
class CrossMambaFusion_SS2D_SSM(nn.Module):
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        # print("x_rgb,CrossMambaFusion_SS2D_SSM输入每个维度的大小为:", x_rgb.shape)
        # print("x_e,CrossMambaFusion_SS2D_SSM输入每个维度的大小为:", x_e.shape)
        x_rgb = self.in_proj(x_rgb)
        # print("x_rgb,CrossMambaFusion_SS2D_SSMin_proj每个维度的大小为:", x_rgb.shape)
        x_e = self.in_proj_modalx(x_e)
        # print("x_e,CrossMambaFusion_SS2D_SSMin_proj_modalx每个维度的大小为:", x_e.shape)
        B, H, W, D = x_rgb.shape
        if self.d_conv > 1:
...
class ConMB_SS2D(nn.Module):
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            x_rgb_exitation = self.fc1(x_rgb_squeeze).view(b, d, 1, 1).permute(0, 2, 3,
                                                                               1).contiguous()  # b, 1, 1, d
