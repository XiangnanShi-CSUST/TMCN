...
class VSSBlock(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        # print("_forwardinput,每个维度的大小为:", input.shape)
        x = input + self.drop_path(self.op(self.norm(input)))
        # print("x,每个维度的大小为:", x.shape)
        if self.mlp_branch:
            x = x + self.drop_path(self.mlp(self.norm2(x)))  # FFN
            # print("if self.mlp_branch:x,每个维度的大小为:", x.shape)
        # print("returnx,每个维度的大小为:", x.shape)
        return x

    def forward(self, input: torch.Tensor):
        # print("input,每个维度的大小为:", input.shape)
