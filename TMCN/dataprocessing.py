...
import setproctitle
setproctitle.setproctitle("cddfuse")
...
def Im2Patch(img, win, stride=1):
    # 该函数的目的是将输入的图像按照指定的窗口大小和步长转换为图像块的形式，方便后续处理。
    k = 0
    endc = img.shape[0]
    endw = img.shape[1]
    endh = img.shape[2]
...
def is_low_contrast(image, fraction_threshold=0.1, lower_percentile=10,
                    upper_percentile=90):
    # 这个函数的主要目的是提供一种判断图像对比度的方法，判断依据是图像的亮度范围占整体亮度的比例是否小于阈值。如果是低对比度的图像，则返回 True，否则返回 False。
...
h5f = h5py.File(os.path.join(data_name+'_imgsize_'+str(img_size)+"_stride_"+str(stride)+'.h5'),
                    'w')
# assert len(IR_files) == len(VIS_files)
...
with h5py.File(os.path.join('data',
                                 data_name+'_imgsize_'+str(img_size)+"_stride11_"+str(stride)+'.h5'),"r") as f:
    for key in f.keys():
